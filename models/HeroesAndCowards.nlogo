...
to setup
  ...
  create-turtles number [
    ...
    let turtles-excluding-friend [ other turtles ] of friend
    set enemy one-of other turtles-excluding-friend
  ]
  reset-ticks
end
...
Wilensky, U & Rand, W. (2015). Introduction to Agent-Based Modeling: Modeling Natural, Social and Engineered Complex Systems using NetLogo. Cambridge, Ma. MIT Press.
...
The rules of this model are that there are two basic personality types.  All agents in the model choose a friend and an enemy.  If they are in a COWARDLY mood, then the agent tries to keep their friend between them and their enemy.  If they are in a BRAVE mood, then the agent tries to stay between their enemy and their friend.
...
Can you identify the different patterns described in the textbook?  For instance, can you find a frozen behavior?  A dot behavior?  A slinky behavior?  A spiral behavior?
...
You can create buttons that capture interesting patterns of behaviors by using the RANDOM-SEED function in NetLogo.  First set the RANDOM-SEED to different values. PRESS SETUP then GO and observe the behaviors. Create your own buttons that produce interesting behaviors.
...
* Stonedahl, F., Wilensky, U. & Rand, W. (2010). NetLogo Heroes and Cowards Model.  http://ccl.northwestern.edu/netlogo/models/IABMTextbook/HeroesandCowards.  Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
